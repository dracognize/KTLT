...
namespace base {
	...
	struct HashTable {
		...
		private:
			...
			using node_allocator = typename std::allocator_traits<t_Allocator>::template rebind_alloc<Node>;
		...
		public:
			...
			struct Iterator {
					...
					constexpr Iterator(const Iterator<T_UNode, T_UValue>& other) noexcept :
						_bucket(other._bucket), _buckets_end(other._buckets_end), _node(other._node) {
					}

					friend constexpr auto operator==(const Iterator& a, const Iterator& b) -> bool = default;
			};
			...
			constexpr explicit HashTable(const t_Hash& hasher, const t_KeyEqual& equal,
										 const t_Allocator& alloc = t_Allocator{}) noexcept :
				_node_alloc(alloc), _bucket_count(DEFAULT_BUCKET_COUNT), _hasher(hasher), _key_equal(equal) {
				_buckets = _alloc_buckets(_bucket_count);
			}
			...
			constexpr HashTable(size_type bucket_hint, const t_Hash& hasher = t_Hash{},
								const t_KeyEqual& equal = t_KeyEqual{}, const t_Allocator& alloc = t_Allocator{}) :
				_node_alloc(alloc),
				_bucket_count(bucket_hint > DEFAULT_BUCKET_COUNT ? bucket_hint : DEFAULT_BUCKET_COUNT), _hasher(hasher),
				_key_equal(equal) {
				_buckets = _alloc_buckets(_bucket_count);
			}

			constexpr HashTable(const HashTable& other) :
				_node_alloc(alloc_traits::select_on_container_copy_construction(other._node_alloc)),
			...
			constexpr HashTable(HashTable&& other) noexcept :
				_node_alloc(std::move(other._node_alloc)), _buckets(other._buckets), _bucket_count(other._bucket_count),
				_size(other._size), _max_load_factor_v(other._max_load_factor_v), _hasher(std::move(other._hasher)),
			...
			constexpr HashTable(std::initializer_list<value_type> init, const t_Hash& hasher = t_Hash{},
								const t_KeyEqual& equal = t_KeyEqual{}, const t_Allocator& alloc = t_Allocator{}) :
				_node_alloc(alloc), _bucket_count(DEFAULT_BUCKET_COUNT), _hasher(hasher), _key_equal(equal) {
				_buckets = _alloc_buckets(_bucket_count);
				reserve(init.size());
			...
			constexpr auto begin() noexcept -> iterator {
				return _make_iterator(_buckets, _buckets + _bucket_count, _buckets ? *_buckets : nullptr);
			}

			constexpr auto begin() const noexcept -> const_iterator {
				return _make_const_iterator(_buckets, _buckets + _bucket_count, _buckets ? *_buckets : nullptr);
			}
			...
			constexpr auto find(const key_type& key) const -> const_iterator {
				...
				while (cur) {
					if (_key_equal(cur->_value.first, key))
						return const_iterator(_buckets + idx, _buckets + _bucket_count, static_cast<const Node*>(cur));
					cur = cur->_next;
				}
				return end();
			}
			...
			constexpr auto load_factor() const noexcept -> float {
				return _bucket_count ? static_cast<float>(_size) / static_cast<float>(_bucket_count) : 0.0f;
			}
		...
		private:
			...
			template <class... T_Args>
			constexpr auto _create_node(T_Args&&... args) -> Node* {
			...
			constexpr auto _make_const_iterator(Node** bucket, Node** end, Node* first_node) const -> const_iterator {
				if (first_node)
	...
	constexpr auto swap(HashTable<t_Key, t_Value, t_Hash, t_KeyEqual, t_Alloc>& a,
