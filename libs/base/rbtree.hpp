...
namespace base {
	template <class t_ValueTypes,
			  class t_ComparatorTypes = std::less<t_ValueTypes>,
	...
	struct RedBlackTree {
			struct Node {
					...
					constexpr explicit Node(const t_ValueTypes &val)
						: _value(val), _parent(nullptr), _left(nullptr), _right(nullptr),
						  _isBlack(false) {};
			};
			...
			template <class T_Node, class T_Ref, class T_Ptr> struct TreeIterator {
					...
					constexpr TreeIterator(T_Node *n, T_Node *nil_ptr) noexcept
						: _node(n), _nil(nil_ptr) {
					}
					...
					friend constexpr auto operator==(const TreeIterator &a, const TreeIterator &b)
						-> bool = default;
			};
		...
		public:
			...
			constexpr explicit RedBlackTree(const comparator_type &comp,
			...
			constexpr RedBlackTree(T_InputIt first,
								   T_InputIt last,
			...
			constexpr RedBlackTree(RedBlackTree &&other) noexcept
				: _nil(&_header), _size(std::exchange(other._size, 0)),
				  _comp(std::move(other._comp)), _alloc(std::move(other._alloc)) {
			...
			constexpr auto operator=(RedBlackTree &&other) noexcept(
				std::is_nothrow_move_assignable_v<comparator_type>
				&& std::is_nothrow_move_assignable_v<node_allocator>) -> RedBlackTree & {
				if (this != &other) {
					clear();
			...
			constexpr auto
			swap(RedBlackTree &other) noexcept(std::is_nothrow_swappable_v<comparator_type>
											   && std::is_nothrow_swappable_v<node_allocator>)
				-> void {
				if (this == &other)
					return;
	...
	constexpr auto
	swap(RedBlackTree<t_ValueTypes, t_ComparatorTypes, t_Alloc> &a,
