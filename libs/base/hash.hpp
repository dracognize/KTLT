...
namespace base {
	...
	struct hash<BasicString<t_Char, t_Traits, t_Alloc>> {
			[[nodiscard]] usize
			operator()(const BasicString<t_Char, t_Traits, t_Alloc> &key) const noexcept {
				return murmur::hash64(
					key.data(), static_cast<i32>(key.size() * static_cast<usize>(sizeof(t_Char))));
			}
	};

	template <class t_Type, usize t_Size> struct hash<Array<t_Type, t_Size>> {
	...
	template <class t_Type, class t_Allocator> struct hash<Vector<t_Type, t_Allocator>> {
	...
	template <class t_Type, class t_Allocator> struct hash<ForwardList<t_Type, t_Allocator>> {
			[[nodiscard]] usize
			operator()(const ForwardList<t_Type, t_Allocator> &key) const noexcept {
				usize h = 0;
				for (const auto &elem : key) {
					h ^= hash<t_Type>{}(elem) + 0x9e3779b9 + (h << 6) + (h >> 2);
				}
	...
	template <class t_Type, class t_Allocator> struct hash<List<t_Type, t_Allocator>> {
