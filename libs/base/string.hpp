...
namespace base {
	template <class t_Char, class t_Traits = std::char_traits<t_Char>, class t_Allocator = std::allocator<t_Char>>
	class BasicString {
		public:
		...
		private:
			static constexpr size_type SSO_Capacity = (sizeof(pointer) + sizeof(size_type)) / sizeof(value_type) - 1;
		...
		public:
			...
			constexpr BasicString(const value_type* s, size_type n, const t_Allocator& alloc = t_Allocator()) :
				_alloc(alloc) {
				if (n <= SSO_Capacity) {
					_isLong = false;
					...
					_isLong				 = true;
					_storage._long._data = std::allocator_traits<t_Allocator>::allocate(_alloc, n + 1);
			...
			constexpr ~BasicString() {
				if (_isLong) {
					std::allocator_traits<t_Allocator>::deallocate(_alloc, _storage._long._data,
																   _storage._long._cap + 1);
				}
			}
			...
			constexpr BasicString(const BasicString& other) :
				_alloc(std::allocator_traits<t_Allocator>::select_on_container_copy_construction(other._alloc)) {
			...
			constexpr BasicString(BasicString&& other) noexcept :
				_storage(other._storage), _alloc(std::move(other._alloc)), _isLong(other._isLong) {
				other._isLong					 = false;
			...
			[[nodiscard]] constexpr auto operator==(const BasicString& other) const noexcept -> bool {
				return size() == other.size() && t_Traits::compare(data(), other.data(), size()) == 0;
			}
	};
...
struct std::formatter<base::BasicString<char, t_Traits, t_Alloc>> : formatter<std::string_view> {
		auto format(const base::BasicString<char, t_Traits, t_Alloc>& s, format_context& ctx) const {
...
struct std::formatter<base::BasicString<char8_t, t_Traits, t_Alloc>> : formatter<std::string_view> {
		auto format(const base::BasicString<char8_t, t_Traits, t_Alloc>& s, format_context& ctx) const {
			return formatter<std::string_view>::format({reinterpret_cast<const char*>(s.data()), s.size()}, ctx);
		}
};
