...
namespace base {
	template <class t_Type, class t_Allocator = std::allocator<t_Type>> struct ForwardList {
			struct Node {
					t_Type _value{};
					Node *_next = nullptr;
			...
			template <class T_Node, class T_Ref, class T_Ptr> struct ForwardIterator {
					...
					constexpr ForwardIterator(
						const ForwardIterator<T_UNode, T_URef, T_UPtr> &other) noexcept
						: _node(other._node) {
					}

					friend constexpr auto operator==(const ForwardIterator &a,
													 const ForwardIterator &b) -> bool = default;
			};
		...
		public:
			...
			constexpr explicit ForwardList(const size_type &count,
			...
			constexpr ForwardList(T_InputIt first,
								  T_InputIt last,
								  const t_Allocator &alloc = t_Allocator{})
				: _alloc(alloc) {
				Node *prev = &_head;
			...
			constexpr ForwardList(std::initializer_list<value_type> init,
								  const t_Allocator &alloc = t_Allocator{})
				: _alloc(alloc) {
				Node *prev = &_head;
				for (auto &val : init) {
			...
			template <std::input_iterator T_InputIt>
			constexpr auto insert_after(const_iterator pos, T_InputIt first, T_InputIt last)
				-> iterator {
				auto *node = const_cast<Node *>(pos._node);
				for (; first != last; ++first) {
					auto *n = _create_node(*first);
			...
			constexpr auto insert_after(const_iterator pos, std::initializer_list<value_type> ilist)
				-> iterator {
			...
			constexpr auto
			swap(ForwardList &other) noexcept(std::is_nothrow_swappable_v<node_allocator>) -> void {
