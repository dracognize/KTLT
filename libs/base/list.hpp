...
namespace base {
	...
	struct List {
			...
			struct ListIterator {
					...
					constexpr ListIterator(const ListIterator<T_UNode, T_URef, T_UPtr>& other) noexcept :
						_node(other._node) {
					}

					friend constexpr auto operator==(const ListIterator& a, const ListIterator& b) -> bool = default;
			};
		...
		public:
			...
			constexpr explicit List(const size_type& count, const value_type& value,
			...
			constexpr explicit List(const size_type& count, const t_Allocator& alloc = t_Allocator{}) : _alloc(alloc) {
				_head._prev = &_head;
				_head._next = &_head;
				Node* tail	= &_head;
				for (size_type i = 0; i < count; ++i) {
			...
			constexpr List(T_InputIt first, T_InputIt last, const t_Allocator& alloc = t_Allocator{}) : _alloc(alloc) {
				_head._prev = &_head;
				_head._next = &_head;
				Node* tail	= &_head;
			...
			constexpr List(std::initializer_list<value_type> init, const t_Allocator& alloc = t_Allocator{}) :
				_alloc(alloc) {
				_head._prev = &_head;
				_head._next = &_head;
				Node* tail	= &_head;
			...
			// --- Swap ---
			constexpr auto swap(List& other) noexcept(std::is_nothrow_swappable_v<node_allocator>) -> void {
				if (this == &other)
					return;
			...
			template <class T_Compare>
			constexpr auto merge(List& other, T_Compare comp) -> void {
			...
			template <class T_Compare>
			constexpr auto sort(T_Compare comp) -> void {
				if (_size <= 1)
					return;
			...
			constexpr auto splice(const_iterator pos, List& other, const_iterator first, const_iterator last) -> void {
				auto* first_node = const_cast<Node*>(first._node);
				auto* last_node	 = const_cast<Node*>(last._node);
	...
	constexpr auto swap(List<t_Type, t_Allocator>& a, List<t_Type, t_Allocator>& b) noexcept(noexcept(a.swap(b)))
		-> void {
		a.swap(b);
	}
} // namespace base
